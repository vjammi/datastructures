package io.dev.java.reactive.service;

import io.dev.java.reactive.exception.UserTransformationException;
...
public class ReactiveService {
    ...
    public Flux<User> getUsers() {
        Flux<User> users = repository.getUsers()
                .map(user -> returnTransformedUserOrThrowException(user))
                .onErrorResume(error -> {
    ...
    public User returnTransformedUserOrThrowException(User user) {
        try {
            return toUpper(user);
        } catch (Exception e) {
    ...
    private User toUpper(User user) throws UserTransformationException {
        ...
        if (user.getName().equalsIgnoreCase("Dan")) {
            throw new UserTransformationException("IllegalArgumentException for user");
        }
