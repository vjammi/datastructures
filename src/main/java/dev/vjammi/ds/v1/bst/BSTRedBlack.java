...
public class BSTRedBlack<Key extends Comparable<Key>, Value> {
    ...
    // put the key-value pair in the subtree rooted at h
    ...
    private Node rotateRight(Node h) {
        ...
        h.left = x.right;
        x.right = h;
        x.color = h.color;  // x.color = x.right.color;
        h.color = RED;      // x.right.color = RED;
        x.size = h.size;
        h.size = size(h.left) + size(h.right) + 1;
        return x;
    }
